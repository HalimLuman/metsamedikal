...
const CarouselBanner = ({title, description, image, bgColor, style}) => {
  return (
    <CarouselItem className={`${bgColor}-100`}>
          <div className={`container flex flex-col  ${style === 'left' ? 'lg:flex-row' : 'lg:flex-row-reverse'} items-center justify-evenly gap-10 lg:gap-20`}>
            <div className={`flex flex-col text-center ${style === 'left' ? 'lg:text-left items-start' : 'lg:text-right items-end'} `}>
              ...
              <Button className={`w-full lg:w-[150px] mt-6 ${bgColor}-600 text-white hover:${bgColor}-700 rounded-md transition duration-300`}>
                Shop Now
              </Button>
            </div>
